...
func TestDataQueryProtocol(t *testing.T) {
	...
	err = protocol2.PutQueryHistory(hexutil.Encode(hashOfReq), dqrequest, time.Now().Unix())
	assert.NoError(t, err)
	...
	err = protocol2.PutQueryHistory(hexutil.Encode(hashOfOldReq), dqrequestold, dqrequestold.Timestamp)
	assert.NoError(t, err)
	assert.Len(t, protocol2.queryHistory, 2)
