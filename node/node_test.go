...
func createNode(t *testing.T, port string, searchDB string, blockchainDBPath string) *Node {
	...
	storageEngine, err := storage.New(blockchainDB, filepath.Join("", "data_storage"), true, "123", 8, host.ID().String())
	assert.NoError(t, err)
	...
	storageProtocol, err := storageprotocol.New(host, storageEngine, nil, false)
	assert.NoError(t, err)
