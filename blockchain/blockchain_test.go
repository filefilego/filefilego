...
func TestPerformStateUpdateFromDataPayload(t *testing.T) {
	...
	txWithChannelPayload.SetTransactionFees("0x" + fees.Text(16))
	err = blockchain.performStateUpdateFromDataPayload(txWithChannelPayload)
	assert.NoError(t, err)
	...
	files, err := blockchain.GetFilesFromEntryOrFolderRecursively(childsOfSubchannelffg[0].NodeHash, 0, 1000, "asc")
	assert.NoError(t, err)
