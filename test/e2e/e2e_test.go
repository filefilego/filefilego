...
func TestE2E(t *testing.T) {
	...
	stats1, err := fileDownloader1Client.DownloadFile(context.TODO(), downloadContract.Contract.ContractHash, file1UploadResponse.FileHash, uint64(file1UploadResponse.Size), 0, 8)
	assert.NoError(t, err)
	assert.Equal(t, "started", stats1)
	stats2, err := fileDownloader1Client.DownloadFile(context.TODO(), downloadContract.Contract.ContractHash, file2UploadResponse.FileHash, uint64(file2UploadResponse.Size), 0, 8)
	assert.NoError(t, err)
